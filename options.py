...
class Option(NetOption):
    def __init__(self, conf_path):
        ...
        self.mn_perc = self.conf['mn_perc']
        self.DSG_alpha = self.conf['DSG_alpha']
        self.DSG_beta = self.conf['DSG_beta']

    def set_save_path(self):
        self.save_path = self.save_path + "log_{}_bs{:d}_lr{:.4f}_TELCNN_baseline_opt{}_qw{:d}_qa{:d}_epoch{}_{}_{}/".format(
            self.dataset, self.batchSize, self.lr, self.opt_type, self.qw, self.qa,
            self.nEpochs, self.experimentID, time.time())

        if os.path.exists(self.save_path):
            print("{} file exist!".format(self.save_path))
            action = input(
                "Select Action: d (delete) / q (quit):").lower().strip()
            act = action
            if act == 'd':
