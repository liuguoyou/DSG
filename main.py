...
class GeneratorCifar(nn.Module):
    def __init__(self, options=None, conf_path=None):
        ...
        self.label_emb = nn.Embedding(
            self.settings.nClasses, self.settings.latent_dim)
...
class GeneratorImagenet(nn.Module):
    def __init__(self, options=None, conf_path=None):
        ...
        super(GeneratorImagenet, self).__init__()
        ...
        self.conv_blocks2_5 = nn.BatchNorm2d(
            self.settings.channels, affine=False)

    def forward(self, z, labels):
        out = self.l1(z)
        out = out.view(out.shape[0], 128, self.init_size, self.init_size)
        img = self.conv_blocks0_0(out, labels)
        img0 = nn.functional.interpolate(img, scale_factor=2)
        img = self.conv_blocks1_0(img0)
        img = self.conv_blocks1_1(img, labels)
        img = self.conv_blocks1_2(img)
        img1 = nn.functional.interpolate(img, scale_factor=2)
        img = self.conv_blocks2_0(img1)
...
class ExperimentDesign:
    def __init__(self, generator=None, options=None, conf_path=None, arc='resnet18'):
        self.settings = options or Option(conf_path)
    ...
    def set_logger(self):
        ...
        file_handler = logging.FileHandler(os.path.join(
            self.settings.save_path, "train_test.log"))
    ...
    def _set_model(self):
        if self.settings.dataset in ["cifar100"]:
            ...
            self.model_teacher = ptcv_get_model(
                'resnet20_cifar100', pretrained=True)
            ...
            print(self.arc)
            arc2name = {
                'shufflenet': 'shufflenet_g1_w1',  # shufflenet_g1_w1
                'mobilenetv2': 'mobileNetv2_w1',  # mobileNet v2
            ...
            try:
                model_name = arc2name[self.arc]
                print(model_name)
                self.model = ptcv_get_model(model_name, pretrained=True)
                self.model_teacher = ptcv_get_model(
                    model_name, pretrained=True)
                self.model_teacher.eval()
            except Exception as e:
                print("Failed to load model, check the arc value")
                print('# Error msg:', e)
        else:
    ...
    def run(self):
        ...
        try:
            for epoch in range(self.start_epoch, self.settings.nEpochs):
                ...
                self.logger.info("#==>Best Result is: Top1 Error: {:f}, Top5 Error: {:f}".format(
                    best_top1, best_top5))
...
def main():
    ...
    parser.add_argument('--arc', type=str,
                        metavar='arc',
                        help='architecture', default='resnet18')
    ...
    option.experimentID = option.experimentID + \
        "{:0>2d}_repeat".format(args.id)

    if option.dataset in ["cifar100"]:
        generator = GeneratorCifar(option)
    elif option.dataset in ["imagenet"]:
        generator = GeneratorImagenet(option)
    else:
        assert False, "invalid data set"

    experiment = ExperimentDesign(
        generator, option, arc=args.arc)
    experiment.run()
